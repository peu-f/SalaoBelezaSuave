...
function listarAvaliacoes() {
  ...
  avaliacoes.forEach((avaliacao, index) => {
      ...
      card.innerHTML = `
          ...
          <div class="buttons">
              <button type="button" class="btn" onclick="publicarAvaliacao(${index})" id="botaodesativado">Publicar avaliação</button>
              <button class="btn" id="deleteAva" onclick="excluirAvaliacao(${index})">Excluir avaliação</button>
          </div>
      `;
      container.appendChild(card);
  });              
  ...
  function publicarAvaliacao(index) {
    ...
    if (index < avaliacoes.length) {
        ...
        // Encontra o botão específico dessa avaliação
        // Como você recarrega a lista, não precisa modificar o botão aqui
        // A modificação será feita quando a lista for recarregada
